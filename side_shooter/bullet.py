...
class Bullet(pygame.sprite.Sprite):
  def __init__(self, pos, surf, direction, groups):
    ...
    # flip bullet direction if facing left
    if direction.x < 0:
...
class FireAnimation(pygame.sprite.Sprite):
  def __init__(self, entity, surf_list, direction, groups):
    ...
    self.frames = [pygame.transform.flip(frame, True, False) if direction.x < 0 else frame for frame in self.frames]
    ...
    y_offset = 10 if entity.ducking else -16
